...
Após aplicar o estilo e garantir que o ScrollView e os botões operam visualmente, salve o progresso:
```bash
git add app/\(tabs\)/_layout.tsx app/\(tabs\)/add.tsx
git commit -m "feat(ui): implement medication registration form screen and layout"
```
